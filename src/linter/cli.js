...
export default class Cli {
  ...
  static async getFiles(dir) {
    ...
    const files = await Promise.all(entries.map((res) => {
      ...
      return res.isDirectory() ? this.getFiles(resPath) : resPath;
    }));
    return Array.prototype.concat(...files).filter(f => 
      ['.js', '.html', '.css'].includes(path.extname(f))
    );
  }
  ...
  static report(file, violations) {
    ...
    violations.forEach(v => {
      console.log(`  [${v.ruleId}] Line ${v.line}, Col ${v.column}: ${v.message}`);
    });
  }
}
